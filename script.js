...
function operator(op) {
  ...
  if (currentNumber == "") {
    ...
    document.getElementById("input_field").value = oldval.replaceAt(
      oldval.length - 1,
      op
    );
    expression[--index] = op;
...
function calculate(firstOperand, secondOperand, operator) {
  switch (operator) {
    ...
    case "-":
      if (firstOperand > secondOperand) return firstOperand - secondOperand;
      else return secondOperand - firstOperand;
    case "*":
      return firstOperand * secondOperand;
    case "/":
      return firstOperand / secondOperand;
